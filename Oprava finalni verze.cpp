...
void strom(string &odpoved, string &spravnaodpoved, int &gold){
	spravnaodpoved = "umela inteligence";
	cout << "Po ceste si narazil na stareho robota ktery pro tebe ma nabidku, ktera se neodmita...\n";
	cout << "Robot: Kdyz uhadnes mou hadanku mam pro tebe odmenu kterou jen tak nenajdes \n";
	cout << "Hadanka zni: Jsem stvořena lidmi,však lidské slabosti neznám.Nemám duši, a přece myslím.Nepotřebuji oči, přesto vidím.Moje tělo je tiché, ale mé výpočty hlučné.Kdo jsem?(2 slova, mala pismena): ";
	cin.ignore();
	getline (cin, odpoved);
	if(odpoved == spravnaodpoved || odpoved == "Umela inteligence"){
		cout << "Gratuluji tato odpoved je spravna ziskavas ode me 20 goldu";
		gold = gold + 20;
		}else{cout << "Bohuzel tato odpoved je spatna budes muset pokracovat dal bez odmeny";
		}}
...
void vesnice(int &ziv, int &maxziv, int &eng, int &utokl, int &utokt, int &utokk, int &gold, int &maxeng) {
    ...
    while (true) {
        ...
        cout << "Tvoje zivoty: "<<ziv;
        cout << "\nTvoje energie: "<<eng;
        cout << "\nLehky utok: "<<utokl;
        cout << "\nTezky utok: "<<utokt;
        cout << "\nKriticky utok: "<<utokk;
        cout << "\nStav goldu: " << gold << endl;
        cout << "1 - Doplneni zivotu (3 goldy)" << endl;
        cout << "2 - Zvyseni maximalnich zivotu o 5 (2 goldy)" << endl;
        cout << "3 - Zvyseni energie o 5 (2 goldy)" << endl;
        cout << "4 - Zvyseni utoku o 1 (3 goldy)" << endl;
        ...
        switch (volba) {
            ...
            case 4:
                if (gold >= 3) {
                    gold -= 3;
                     int volbautok;
                  do{  cout<< "Jaky utok chces vylepsit? ";
                    cout << "1-Lehky utok\n";
                    cout << "2-Tezky utok\n";
                    cout << "3-Kriticky utok\n";
                    cout << "Tvoje volba: ";
                    cin >> volbautok;
                    switch(volbautok){
                    	cout << "Lehky utok byl vylepsen o 1 dmg\n";
                    case 1: utokl = utokl +1;
                    break;
                    	cout << "Tezky utok byl vylepsen o 1 dmg\n";
                    case 2: utokt = utokt +1;
                    break;
                    	cout << "Tezky utok byl vylepsen o 1 dmg\n";
                    case 3: utokk = utokk + 1;
                    break;
                    default: cout << "Neplatna volba vyber si znovu.";	}}while(volbautok <= 0 || volbautok > 3);
...
void classa(int &volba, int &finalvolba, int &utokl, int &utokt, int &utokk, int &ziv, int &maxziv, int &eng, int &maxeng, int &gold)
 {
    do {
    ...
    } while (finalvolba == 0 || finalvolba > 3);
}
...
int main() {
    ...
    classa(volbaves, finalvolba, utokl, utokt, utokk, ziv, maxziv, eng, maxeng, gold);
    cout << "Napis jmeno sveho charakteru (jednoslovne): ";
    cin >> jmeno;
    ...
    cout << " z tovarny chronoforge, takze se tyto pripady snazi zamest aby si verejnost dale roboty kupovala a Chronobotuv plan vysel.";
...
fightMonster(ziv, maxziv, eng, utokl, utokt, utokk, gold, maxeng, 3);
fightMonster(ziv, maxziv, eng, utokl, utokt, utokk, gold, maxeng, 2);
strom(odpoved, spravnaodpoved, gold);
